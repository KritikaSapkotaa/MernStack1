...
const createOrder = async (DataTransfer, userId) => {
...
const deleteOrder = async (id, user) => {
    ...
    if (order.user._id != user._id && !user.roles.include(ADMIN)) {
        throw {
            statusCode: 403,
            message: "Access denied.",
        };
...
const orderPaymentViaKhalti = async (id, user) => {
    ...
    const transactionId = crupto.randomUUID();
...
const orderPaymentViaStripe = async (id, user) => {
    ...
    const orderPayment = await Payemnt.create({
        amount: order.totalPrice,
        method: "card",
        transactionId,
    });
...
const confirmOrderPayment = async (id, status, user) => {
    const order = await grtOrderById(id);
...
const getOrdersOfMerchant = async (merchantId) => {
    const orders = await Order.agggregate([
        {
            $lookup: {
                from: "products",
                localField: "orderItems.product",
